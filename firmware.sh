...
curl -s -L -O ${rwlegacy_source}${seabios_file}.md5 $http_proxy_address
curl -s -L -O ${rwlegacy_source}${seabios_file}	$http_proxy_address
...
if [ "$preferUSB" = true  ]; then
    ...
    if [[ "$device" = "swanky" ]]; then
    	curl -s -L -o bootorder "${cbfs_source}bootorder.usb2" $http_proxy_address
    else
	curl -s -L -o bootorder "${cbfs_source}bootorder.usb" $http_proxy_address
    fi
    if [ $? -ne 0 ]; then
...
if [ "$useHeadless" = true  ]; then
    curl -s -L -O "${cbfs_source}${hswbdw_headless_vbios}" $http_proxy_address
    if [ $? -ne 0 ]; then
...
curl -s -L -O "${firmware_source}${coreboot_file}" $http_proxy_address
curl -s -L -O "${firmware_source}${coreboot_file}.sha1" $http_proxy_address
...
#addPXE?
if [ "$addPXE" = true  ]; then
    curl -s -L -O "${cbfs_source}${pxe_optionrom}" $http_proxy_address
    if [ $? -ne 0 ]; then
...
the touchpad firmware, otherwise the touchpad will not work."
    ...
    if [[ "$REPLY" = "y" || "$REPLY" = "Y" ]] ; then
        ...
        curl -s -LO "${other_source}${touchpad_eve_fw}" $http_proxy_address
        curl -s -LO "${other_source}${touchpad_eve_fw}.sha1" $http_proxy_address
        #verify checksum on downloaded file
...
if [[ "$confirm" = "Y" || "$confirm" = "y" ]]; then
    ...
    #download shellball
    curl -s -L -o /tmp/shellball.rom ${shellball_source}shellball.${device}.bin $http_proxy_address;
...
curl -s -L -O ${bootstub_source}/${bootstub_payload_baytrail} $http_proxy_address
curl -s -L -O ${bootstub_source}/${bootstub_payload_baytrail}.md5 $http_proxy_address
...
if [[ "$REPLY" = "y" || "$REPLY" = "Y" ]]; then
    ...
    curl -s -L -o bootorder ${cbfs_source}/bootorder.emmc $http_proxy_address
fi
